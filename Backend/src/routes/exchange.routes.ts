import { Router } from "express";
import { createListing, getMarketplaceListings, executeTrade, cancelListing } from "../controllers/exchange.controller";
import { authenticate } from "../middleware/auth.middleware";

const router = Router();

// Browse active listings (Any authenticated user)
router.get("/listings", authenticate, getMarketplaceListings);

// Create a new listing (Seller)
router.post("/listings", authenticate, createListing);

// Buy a listing (Buyer)
router.post("/listings/:id/buy", authenticate, executeTrade);

// NEW: Cancel/Delete a listing (Seller only - enforced in controller)
router.delete("/listings/:id", authenticate, cancelListing);

export default router;